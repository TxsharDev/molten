...
\subsubsection{H100 Real-Model Validation}

On an NVIDIA H100 SXM, we benchmark Molten's fusion analysis on real
Qwen2.5-7B (7.6B parameters) RMSNorm layers. The model has 57 RMSNorm
layers with hidden\_size=3584. \texttt{torch.compile} achieves up to
\textbf{3.76$\times$} over eager at seq=2048 on these real model layers,
confirming that the fusion patterns Molten targets deliver significant
speedups on production architectures.
