...
and automatically generates fused CUDA kernels. Molten operates in five stages:
...
equivalent (max error $< 5 	imes 10^{-6}$, 19/19 test configurations across three GPUs).
\end{abstract}
...
Molten's compilation pipeline has five stages, shown in Figure~\ref{fig:pipeline}.
...
\begin{itemize}
  ...
  \item 	extbf{Attention fusion}: generating a single kernel for the
    full Q@K softmax @V attention pattern is the next major fusion target.
    This requires cross-reduction fusion that the current rule-based engine
    does not support.
...
81\% of peak memory bandwidth at prefill. Numerically exact to $10^{-6}$.
